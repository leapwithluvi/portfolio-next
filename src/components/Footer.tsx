      ...
      <div className="max-container flex flex-col gap-24">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-12 px-4 md:px-0">
           ...
           <div className="flex flex-col gap-4 text-right max-w-xs">
              <div className="text-meta uppercase tracking-widest text-accent">{t.footer.status}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border shadow-2xl shadow-accent/5">
          <div className="bg-background p-10 md:p-12 flex flex-col gap-8 group">
             <div className="text-[10px] font-bold uppercase tracking-[0.3em] text-accent/50">{t.footer.navigation}</div>
               ...
               {navLinks.map((link) => (
                 <li key={link.name}>
                   <a href={link.href} className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground hover:text-accent transition-all flex items-center gap-2 group/link">
                     <span className="w-0 group-hover/link:w-3 h-px bg-accent transition-all" />
          ...
          <div className="bg-background p-10 md:p-12 flex flex-col gap-8">
             <div className="text-[10px] font-bold uppercase tracking-[0.3em] text-accent/50">{t.footer.presence}</div>
             <ul className="flex flex-col gap-6">
               <li className="flex flex-col gap-2">
                 <span className="text-base font-bold font-serif">{t.footer.discipline1}</span>
                 <span className="text-[10px] uppercase text-muted-foreground tracking-widest leading-relaxed">Next.js, Express.js, TypeScript</span>
               </li>
               <li className="flex flex-col gap-2">
                 <span className="text-base font-bold font-serif">{t.footer.discipline2}</span>
                 <span className="text-[10px] uppercase text-muted-foreground tracking-widest leading-relaxed">Python, TensorFlow, PyTorch</span>
               </li>
             </ul>
          </div>
 
          <div className="bg-background p-10 md:p-12 flex flex-col gap-8">
             <div className="text-[10px] font-bold uppercase tracking-[0.3em] text-accent/50">{t.footer.connect}</div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-12 border-t border-border gap-8 px-4 md:px-0">
           <div className="text-[10px] font-bold opacity-30 uppercase tracking-[0.4em]">
             &copy; {currentYear} {profile.name} — {t.footer.rights}
           </div>
           <div className="text-[10px] font-bold opacity-30 uppercase tracking-[0.4em] hover:opacity-100 transition-opacity">
             {t.footer.credit}
           </div>
        </div>
      </div>
